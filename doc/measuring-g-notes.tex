...
Earth's center accelerates toward the Sun, Moon, and test mass:
\begin{equation}
M_E\,\ddot{\vb{R}}_E
  = -\frac{G M_S\, M_E}{\lvert\vb{R}_E - \vb{R}_S\rvert^3}
    (\vb{R}_E - \vb{R}_S)
  \;-\; \frac{G M_M\, M_E}{\lvert\vb{R}_E - \vb{R}_M\rvert^3}
    (\vb{R}_E - \vb{R}_M)
  \;-\; \frac{G\, m\, M_E}{\lvert\vb{R}_E - \vb{x}\rvert^3}
    (\vb{R}_E - \vb{x})
\label{eq:earth}
\end{equation}

The common factor~$M_E$ divides out of every term, so Earth's
acceleration is independent of its own mass --- as guaranteed by the
equivalence principle. The last term is the gravitational back-reaction
of the test mass; after dividing by~$M_E$ it contributes an acceleration
$Gm/r^2 \sim 10^{-26}$~m/s$^2$ for $m \sim 1$~kg, negligible compared
to all other terms. It will produce a correction of order
$m/M_E \sim 10^{-25}$ in the relative equation of motion
[Eq.~\eqref{eq:relative}] that we drop hereafter. Every object rigidly
attached to the Earth --- including the sensor housing --- shares this
acceleration.
...
\begin{equation}
m\,\ddot{\vb{r}}
  = -\frac{G(M_E + m)\, m}{\lvert\vb{r}\rvert^3}\,\vb{r}
  \;+\; G M_S\, m \left[
...
The back-reaction noted after Eq.~\eqref{eq:earth} appears only in the
first term, replacing $M_E \to M_E + m$. Since
$m/M_E \sim 10^{-25}$, we set $M_E + m \approx M_E$ hereafter. The
bracketed tidal terms are entirely unaffected.
...
the Earth, the external body, and the test mass, together with the
reaction force of the spring (Newton's third law) and a constraint
(normal) force~$\vb{N}$ exerted by the crust:
\begin{equation}
m_H\,\ddot{\vb{R}}_H
  = -\frac{G M_E\, m_H}{\lvert\vb{d}\rvert^3}\,\vb{d}
  - \frac{G M\, m_H}{\lvert\vb{R}_H - \vb{R}_B\rvert^3}
    (\vb{R}_H - \vb{R}_B)
  + \frac{G\, m\, m_H}{\lvert\vb{s}\rvert^3}\,\vb{s}
  + \vb{N}
  - \vb{F}_{\text{spring}}
\label{eq:app-housing}
\end{equation}
The third term is the gravitational back-reaction of the test mass on
the housing (it pulls the housing toward the test mass along~$\vb{s}$).
Dividing by~$m_H$, this contributes an acceleration $Gm/s^2$; for
$m \sim 1$~kg and $s \sim 1$~cm this is ${\sim}\,7 \times 10^{-7}$~m/s$^2$
--- a static correction to the spring equilibrium, independent of any
external body, that is absorbed into the sensor calibration. We retain
it in Eq.~\eqref{eq:app-rel} to confirm it does not affect the
cancellation, and drop it thereafter.
...
m\,\ddot{\vb{s}}
  ...
  \right]
  \notag \\
  &\quad - \frac{G\, m^2}{\lvert\vb{s}\rvert^3}\,\vb{s}
  \;+\; \left(1 + \frac{m}{m_H}\right)\vb{F}_{\text{spring}}
    + \frac{m}{m_H}\,\vb{N}
\label{eq:app-rel}
\end{align}

The $Gm^2/s^3$ term is the self-gravity between the test mass and the
housing. It is directed along the spring, static, and independent of
the external body~--- it involves only $m$ and $\vb{s}$, not $M$
or~$\vb{R}$. It therefore cannot affect the cancellation of the
direct pull. Its magnitude ($Gm/s^2 \sim 10^{-7}$~m/s$^2$ for
$m \sim 1$~kg, $s \sim 1$~cm) is absorbed into the spring equilibrium
and the sensor calibration. We drop it hereafter.
...
\subsection{Uniaxial sensor constraint}

A real spring-based sensor constrains the test mass to move along a
single axis~$\hat{\vb{n}}$, the measurement direction. A guide rail
(or equivalent mechanism), rigidly attached to the housing, provides a
constraint force~$\vb{F}_\perp$ perpendicular to~$\hat{\vb{n}}$ that
prevents lateral displacement. The spring extension is then
$\vb{s} = s\,\hat{\vb{n}}$, and only the component
$s = \vb{s}\cdot\hat{\vb{n}}$ is transduced into a reading.

This constraint adds a term to the test-mass equation of
motion~\eqref{eq:app-test}:
\begin{equation}
m\,\ddot{\vb{x}}
  = [\text{gravitational terms}]
  + \vb{F}_{\text{spring}}
  + \vb{F}_\perp,
\label{eq:app-constraint}
\end{equation}
where, by construction,
$\vb{F}_\perp \cdot \hat{\vb{n}} = 0$.
Projecting the equation for $\vb{s}$ onto the measurement axis
therefore eliminates~$\vb{F}_\perp$ entirely. The scalar equation
governing the sensor reading is
\begin{equation}
m\,\ddot{s}
  = \hat{\vb{n}}\cdot[\text{all gravitational terms}]
  + F_{\text{spring}},
\label{eq:app-projected}
\end{equation}
which is identical to projecting the unconstrained three-dimensional
equation onto~$\hat{\vb{n}}$. The constraint force has dropped out.

The cancellation of the direct pull is unaffected because it is a
vectorial identity: $\vb{g}(\vb{0}) = \vb{0}$
(Eq.~\ref{eq:app-tidal} at $\vb{s} = \vb{0}$). The projection of
the zero vector onto any axis vanishes ---
$\hat{\vb{n}} \cdot \vb{0} = 0$ --- regardless of the sensor
orientation.

One might worry that the guide rail introduces a new channel for
external-body forces. It does not: the guide rail is rigidly attached
to the housing, which shares the Earth's free-fall acceleration toward
the external body. The constraint force~$\vb{F}_\perp$ therefore
undergoes the same cancellation as all other housing-attached
components. It contains no direct-pull contribution from external
bodies, only tidal corrections perpendicular to~$\hat{\vb{n}}$, which
do not enter the measurement.

In summary, the one-dimensional constraint introduces no new terms in
the sensor reading. The spring measures the
$\hat{\vb{n}}$-component of the force balance, and the cancellation
of the direct pull --- being a vectorial identity --- holds for every
projection.
