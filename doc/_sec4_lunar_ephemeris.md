...
Computing tidal acceleration requires the Moon's geocentric position vector at every instant. This section derives the analytical lunar ephemeris that Pytheas uses.
...
where $\mathbf{R}$ is the Moon's geocentric position vector. The tidal acceleration is vectorial (sensitive to the Moon's angular position) and scales as $a_{\text{tidal}} \propto GM/R^3$. This inverse-cube dependence makes distance knowledge critical:
...
The apogee-to-perigee distance ratio of ~1.14 produces a tidal force ratio of $(406{,}700 / 356{,}500)^3 \approx 1.48$ -- nearly 50% stronger at perigee. At nGal-level gravimetry, even a few hundred kilometres of distance error exceeds the target accuracy. Figure 6 shows this anti-correlation between lunar distance and tidal acceleration amplitude over a synodic month.
...
We need all three coordinates -- longitude, latitude, distance -- to high precision at every moment.

---

## 4.2 Keplerian Orbit Review

The unperturbed two-body orbit is an ellipse with Earth at one focus, parameterised by semi-major axis $a$ and eccentricity $e$ ($\approx 0.055$ for the Moon). Perigee and apogee distances are $a(1-e)$ and $a(1+e)$.

### Orbital elements

Six Keplerian elements specify the orbit: $a$ (size), $e$ (shape), $i$ (inclination; $\approx 5.14°$ to the ecliptic), $\Omega$ (longitude of ascending node), $\omega$ (argument of periapsis), and $M$ (mean anomaly).

### Anomalies

The mean anomaly advances uniformly:

$$
M = M_0 + n(t - t_0),
$$

with mean motion $n = 2\pi/P$. The true anomaly $\nu$ -- the actual angular position measured from perigee -- is obtained via the eccentric anomaly $E$ from **Kepler's equation**:

$$
M = E - e \sin E,
$$

solved iteratively, then

$$
\tan\frac{\nu}{2} = \sqrt{\frac{1+e}{1-e}} \tan\frac{E}{2}, \qquad r = a(1 - e\cos E).
$$

For an isolated two-body system, this would suffice.
...
The Sun makes the lunar problem a three-body system with no closed-form solution (Poincare, 1890s). The qualitative effects of the solar perturbation are:
...
**Eccentricity oscillation.** Solar forcing drives $e$ between ~0.044 and ~0.067.

**Inclination oscillation.** The orbital inclination oscillates between ~4.98° and ~5.30°, correlated with nodal regression.

The difficulty of lunar theory occupied Newton ("his head never ached but with his studies on the Moon"), and subsequently Euler, Clairaut, Laplace, Delaunay, Hill, and Brown -- driven in large part by the practical demands of maritime navigation.
...
Two strategies exist for predicting the Moon's position: numerical integration (JPL DE430/DE440) and analytical perturbation series. Pytheas uses the latter, following **Jean Meeus** (*Astronomical Algorithms*, Chapter 47), which distills the ELP-2000 theory of Chapront-Touze and Chapront to sub-arcminute positional accuracy and ~200 km distance accuracy.

The Moon's ecliptic longitude, latitude, and distance are each expressed as sums of sinusoidal terms whose arguments are integer combinations of five fundamental angles.

### The five fundamental arguments

All are polynomials in $T$ (Julian centuries since J2000.0, where $T = (JD - 2{,}451{,}545.0)/36{,}525$):

**$L'$ -- Moon's mean longitude** (tracks mean orbital motion, ~13.18°/day):

$$
L' = 218.3164477^\circ + 481{,}267.88123421^\circ\, T - 0.0015786^\circ\, T^2 + \frac{T^3}{538{,}841}
$$

**$D$ -- Mean elongation** (Moon-Sun angular separation; $D=0$ at new moon, $180°$ at full moon, ~12.19°/day $\to$ synodic month ~29.53 d):

$$
D = 297.8501921^\circ + 445{,}267.1114034^\circ\, T - 0.0018819^\circ\, T^2 + \frac{T^3}{545{,}868}
$$

**$M$ -- Sun's mean anomaly** (~0.986°/day $\to$ tropical year ~365.25 d):
...
These five arguments have distinct periods, so every lunar perturbation decomposes into sinusoids with integer linear combinations of $(D, M, M', F)$ plus the mean longitude $L'$.
...
where $a_i$ is in units of $10^{-6}$ degrees and the $n_k^{(i)}$ are small integers.

**Ecliptic latitude** (no mean term -- the Moon's mean position is on the ecliptic):

$$
\beta = \frac{1}{10^6} \sum_j c_j \sin(n_1^{(j)} D + n_2^{(j)} M + n_3^{(j)} M' + n_4^{(j)} F)
$$

**Distance** (cosine series -- extrema align with anomaly extrema):

$$
\Delta = 385{,}000.56 \text{ km} + \frac{1}{1000} \sum_k b_k \cos(n_1^{(k)} D + n_2^{(k)} M + n_3^{(k)} M' + n_4^{(k)} F)
$$

where $b_k$ is in metres. Pytheas retains **24 terms** for longitude, **23 for distance**, and **18 for latitude**.
...
|---|-------|-------|----------|-------|-------------------|-----------|----------------------|
| 1 | 0 | 0 | 1 | 0 | +6,288,774 | 6.289° | **Equation of the centre**: elliptical motion (faster at perigee, slower at apogee). |
| 2 | 2 | 0 | $-1$ | 0 | +1,274,027 | 1.274° | **Evection**: Sun modulates the effective eccentricity depending on apsidal-line alignment (Ptolemy, ~2nd c.). |
| 3 | 2 | 0 | 0 | 0 | +658,314 | 0.658° | **Variation**: Moon speeds up near syzygy, slows at quadrature (Tycho Brahe, ~1590). |
| 4 | 0 | 0 | 2 | 0 | +213,618 | 0.214° | Second harmonic of the equation of the centre. |
| 5 | 0 | 1 | 0 | 0 | $-185,116$ | 0.185° | **Annual equation**: varying Sun-Earth distance modulates the solar perturbation over the year. |

The series converges rapidly: the 24th term (coefficient 4,036) contributes only ~0.004° (~14 arcseconds).
...
|---|-------|-------|----------|-------|-------------------|-----------|----------------|
| 1 | 0 | 0 | 0 | 1 | +5,128,122 | 5.128° | **Inclination term**: orbital tilt to ecliptic. |
| 2 | 0 | 0 | 1 | 1 | +280,602 | 0.281° | Eccentricity-inclination coupling. |
...
Figure 7 ranks the longitude perturbation terms by amplitude, illustrating the rapid convergence -- the first few terms capture the dominant physics, while the tail refines the result to sub-arcminute accuracy.
...
Terms involving the Sun's mean anomaly $M$ ($n_2 \neq 0$) must be corrected for the secular decrease in Earth's orbital eccentricity:

$$
E = 1 - 0.002516\, T - 0.0000074\, T^2.
$$

Each coefficient is multiplied by $E^{|n_2|}$. This accounts for Earth's orbit becoming more circular (~$-0.00004$/century in $e$), which modulates the Sun's perturbing effect on terms involving $M$.
...
Beyond the main series, Meeus includes three additive corrections:
...
- **Earth's oblateness**: $+1962 \sin(L' - F)$.
...
The perturbation series yields ecliptic coordinates $(\lambda, \beta, \Delta)$. The tidal formula requires ECEF Cartesian coordinates, obtained via two rotations.
...
Expressing the Moon's position in Cartesian ecliptic coordinates:
...
the ecliptic-to-equatorial rotation about the $x$-axis (vernal equinox direction, shared by both frames) is
...
**Sanity check:** For $\beta = 0$, $\lambda = 90°$: $X_{\text{ECI}} = 0$, $Y_{\text{ECI}} = \Delta\cos\varepsilon$, $Z_{\text{ECI}} = \Delta\sin\varepsilon$ -- the Moon appears above the equatorial plane by the obliquity angle, as expected.
...
The ECEF frame co-rotates with Earth. The angle between the ECI and ECEF $x$-axes is the **GMST** $\theta$ (derived in Section 2). The transformation is a $z$-rotation:
...
The $z$-component is unchanged (rotation about the polar axis).
...
The result is $\mathbf{R}_{\text{Moon}}$ in ECEF, ready for the tidal acceleration formula from Section 6.

---

### Summary

The Moon's position cannot be computed from a simple Keplerian orbit because solar perturbations continuously distort the lunar trajectory. Pytheas uses an analytical perturbation series (Meeus's compilation of ELP-2000): 24+23+18 = 65 sinusoidal terms in five fundamental angles capture ecliptic longitude, distance, and latitude to sub-arcminute accuracy. Two rotations (ecliptic $\to$ equatorial $\to$ ECEF) place the result in Earth-fixed coordinates for tidal calculations.
