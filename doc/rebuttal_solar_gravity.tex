...
Equation~\eqref{eq:geocentric} is written in the non-rotating geocentric
...
Let $\vb{r}'$ denote the test mass position in the lab frame, related
...
uniform rotation). Substituting into Eq.~\eqref{eq:geocentric}, dividing
by~$m$, and expressing all vectors in the lab-frame basis, Newton's
second law in the lab becomes:
\begin{equation}
\boxed{
\ddot{\vb{r}}'
...
\end{equation}
Here
$\vb{a}_{\text{tidal}}' \equiv
  \vb{a}_{\text{tidal}}^{\text{Sun}\prime}
  + \vb{a}_{\text{tidal}}^{\text{Moon}\prime}$
are the tidal accelerations
(Eqs.~\eqref{eq:tidal_sun}--\eqref{eq:tidal_moon}) expressed in the
lab-frame basis, and the two additional terms are the fictitious
accelerations that arise from working in a rotating frame. Since
rotations preserve norms,
$\lvert\vb{r}'\rvert = \lvert\vb{r}\rvert$.

The direct solar and lunar accelerations do \textbf{not} reappear ---
they canceled in Eq.~\eqref{eq:geocentric} before the frame change, and
rotating the coordinate basis cannot restore them.

\subsection*{Equilibrium in the lab}

The test mass sits at rest on Earth's surface:
$\dot{\vb{r}}' = \vb{0}$ and $\ddot{\vb{r}}' = \vb{0}$. The Coriolis
term vanishes. Setting the left-hand side of Eq.~\eqref{eq:lab} to zero
and solving for~$\vb{F}_{\text{spring}}'$:
\begin{equation}
\vb{F}_{\text{spring}}'
  = m\left[
...
\item $\boldsymbol{\omega}\times(\boldsymbol{\omega}\times\vb{r}')
  = -\omega^2\vb{r}'_\perp$: the centrifugal pseudo-acceleration,
  which \emph{reduces} the required spring force (objects weigh less
  at the equator).
\item $-\vb{a}_{\text{tidal}}'$: the tidal perturbations from
  Eqs.~\eqref{eq:tidal_sun}--\eqref{eq:tidal_moon}, expressed in
  lab coordinates.
\end{itemize}

The sensor projects this onto its measurement axis~$\vu{n}$ (fixed in
the lab frame):
\begin{equation}
g_{\text{measured}}
...
terms canceled in Eq.~\eqref{eq:geocentric} \emph{before} the rotation
to the lab frame --- the spring force inherits only the tidal residuals.
This holds for any~$\vu{n}$.

\medskip\noindent
\textit{Remark.} For a \emph{moving} test mass
($\dot{\vb{r}}' \neq \vb{0}$), the Coriolis term
$-2\,\boldsymbol{\omega}\times\dot{\vb{r}}'$ in Eq.~\eqref{eq:lab}
contributes --- this is the origin of the Foucault pendulum deflection
and the Eötvös effect. For the static equilibrium relevant to
gravimetry, it vanishes identically.
...
\subsection*{Tilt--rotation coupling}

Although tilting cannot access the direct pull, it does change
\emph{which harmonic components} of the tidal field the sensor sees.
This tilt--rotation coupling arises because the measurement axis is
fixed in the lab while the tidal field rotates with the sky.

The Sun's tidal acceleration at the surface
($\vb{r}' = r_\oplus\,\vu{z}$) has East--North--Up components
\begin{alignat}{2}
a_E' &= \phantom{(}\frac{3\,G M_S\, r_\oplus}{R^3}
  &&\cos z\;\sin z\;\sin A
\label{eq:tidal_E} \\
a_N' &= \phantom{(}\frac{3\,G M_S\, r_\oplus}{R^3}
  &&\cos z\;\sin z\;\cos A
\label{eq:tidal_N} \\
a_U' &= \phantom{(}\frac{G M_S\, r_\oplus}{R^3}\;
  &&(3\cos^2 z - 1)
\label{eq:tidal_U}
\end{alignat}
...
\begin{equation}
g_{\text{tidal}}
  = \frac{G M_S\, r_\oplus}{R^3}\!\left[
    \tfrac{3}{2}\sin 2z\;\sin\theta\,\cos(A - \varphi)
    \;+\; (3\cos^2 z - 1)\,\cos\theta
  \right]
\label{eq:tilt_coupling}
\end{equation}

The two terms respond differently to Earth's rotation:
\begin{itemize}
\item The \textbf{vertical term} ($\propto \cos\theta$) depends on
  $3\cos^2 z - 1$. Expanding $\cos z$ in the hour angle~$h$, this
  contains harmonics at frequencies~$0$ (long-period, varying with
  the Sun's declination) and~$2\Omega_\oplus$ (semi-diurnal).
\item The \textbf{horizontal term} ($\propto \sin\theta$) depends on
  $\sin 2z\,\cos(A - \varphi)$. This contains harmonics at
  $\Omega_\oplus$ (diurnal) and~$2\Omega_\oplus$ (semi-diurnal), with
  relative amplitudes that depend on the sensor azimuth~$\varphi$
  and the latitude.
\end{itemize}

Tilting therefore \emph{tunes the frequency response}: a vertical sensor
...
The effect is real and observable, but every term in
Eq.~\eqref{eq:tilt_coupling} carries the tidal prefactor
$G M_S\, r_\oplus / R^3 \sim 5 \times 10^{-7}$~m/s$^2$. Varying
...
(which co-moves with the Earth). From Eq.~\eqref{eq:spring}:
\begin{equation}
